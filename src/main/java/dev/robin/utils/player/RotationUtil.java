...
import dev.robin.utils.Location;
...
public class RotationUtil {
    ...
    public static float[] scaffoldRots(double bx, double by, double bz, float lastYaw, float lastPitch, float yawSpeed, float pitchSpeed, boolean random) {
        ...
        float calcPitch = (float)(-(MathHelper.atan2(y2, MathHelper.sqrt_double(x2 * x2 + z * z)) * 180.0 / Math.PI));
        float pitch = RotationUtil.updateRotation(lastPitch, calcPitch, pitchSpeed + RandomUtils.nextFloat((float)0.0f, (float)15.0f));
        float yaw = RotationUtil.updateRotation(lastYaw, calcYaw, yawSpeed + RandomUtils.nextFloat((float)0.0f, (float)15.0f));
        if (random) {
    ...
    public static float rotateToYaw(float yawSpeed, float currentYaw, float calcYaw) {
        float yaw = RotationUtil.updateRotation(currentYaw, calcYaw, yawSpeed + RandomUtils.nextFloat((float)0.0f, (float)15.0f));
        double diffYaw = MathHelper.wrapAngleTo180_float(calcYaw - currentYaw);
        if ((double)(-yawSpeed) > diffYaw || diffYaw > (double)yawSpeed) {
            yaw += (float)((double)RandomUtils.nextFloat((float)1.0f, (float)2.0f) * Math.sin((double)RotationUtil.mc.thePlayer.rotationPitch * Math.PI));
        }
        if (yaw == currentYaw) {
            return currentYaw;
        }
    ...
    public static float rotateToPitch(float pitchSpeed, float currentPitch, float calcPitch) {
        float pitch = RotationUtil.updateRotation(currentPitch, calcPitch, pitchSpeed + RandomUtils.nextFloat((float)0.0f, (float)15.0f));
        if (pitch != calcPitch) {
            pitch += (float)((double)RandomUtils.nextFloat((float)1.0f, (float)2.0f) * Math.sin((double)RotationUtil.mc.thePlayer.rotationYaw * Math.PI));
        }
        if ((double)RotationUtil.mc.gameSettings.mouseSensitivity == 0.5) {
            RotationUtil.mc.gameSettings.mouseSensitivity = 0.47887325f;
        }
    ...
    public static Vector2f calculate(Entity entity, boolean adaptive, double range) {
        ...
        if (!adaptive || RayCastUtil.rayCast((Vector2f)normalRotations, (double)range).typeOfHit == MovingObjectPosition.MovingObjectType.ENTITY) {
        ...
        for (double yPercent = 1.0; yPercent >= 0.0; yPercent -= 0.25) {
            for (double xPercent = 1.0; xPercent >= -0.5; xPercent -= 0.5) {
                for (double zPercent = 1.0; zPercent >= -0.5; zPercent -= 0.5) {
                    ...
                    if (RayCastUtil.rayCast((Vector2f)adaptiveRotations, (double)range).typeOfHit != MovingObjectPosition.MovingObjectType.ENTITY) continue;

package dev.robin.utils.player;

import dev.robin.Client;
import dev.robin.utils.player.EnumFacingOffset;
...
public final class PlayerUtil {
    ...
    public static boolean isBlockBetween(BlockPos start, BlockPos end) {
        ...
        for (int i = 0; i < STEPS - 1; ++i) {
            BlockPos pos;
            Block block;
            if ((x2 += diffX / (double)STEPS) == (double)endX && (y2 += diffY / (double)STEPS) == (double)endY && (z += diffZ / (double)STEPS) == (double)endZ || (block = Client.mc.theWorld.getBlockState(pos = new BlockPos(x2, y2, z)).getBlock()).getMaterial() == Material.air || block.getMaterial() == Material.water || block instanceof BlockVine || block instanceof BlockLadder) continue;
            return true;
        }
        return false;
    }
    ...
    public static Block block(double x2, double y2, double z) {
        return Client.mc.theWorld.getBlockState(new BlockPos(x2, y2, z)).getBlock();
    }
            ...
            for (int y2 = -range; y2 <= range; ++y2) {
                for (int z = -range; z <= range; ++z) {
                    ...
                    for (int x22 = -1; x22 <= 1; x22 += 2) {
                        possibilities.add(new Vec3(Client.mc.thePlayer.posX + (double)x2 + (double)x22, Client.mc.thePlayer.posY + (double)y2, Client.mc.thePlayer.posZ + (double)z));
                    }
                    for (int y22 = -1; y22 <= 1; y22 += 2) {
                        possibilities.add(new Vec3(Client.mc.thePlayer.posX + (double)x2, Client.mc.thePlayer.posY + (double)y2 + (double)y22, Client.mc.thePlayer.posZ + (double)z));
                    }
                    for (int z2 = -1; z2 <= 1; z2 += 2) {
                        possibilities.add(new Vec3(Client.mc.thePlayer.posX + (double)x2, Client.mc.thePlayer.posY + (double)y2, Client.mc.thePlayer.posZ + (double)z + (double)z2));
                    }
                }
            }
        }
    ...
    public static Block getBlock(double d2, double d22, double d3) {
        return Client.mc.theWorld.getBlockState(new BlockPos(d2, d22, d3)).getBlock();
    }
    ...
    public static boolean isInLiquid() {
        ...
        int y2 = (int)Client.mc.thePlayer.getEntityBoundingBox().minY;
        for (int x2 = MathHelper.floor_double(Client.mc.thePlayer.getEntityBoundingBox().minX); x2 < MathHelper.floor_double(Client.mc.thePlayer.getEntityBoundingBox().maxX) + 1; ++x2) {
            for (int z = MathHelper.floor_double(Client.mc.thePlayer.getEntityBoundingBox().minZ); z < MathHelper.floor_double(Client.mc.thePlayer.getEntityBoundingBox().maxZ) + 1; ++z) {
                Block block = Client.mc.theWorld.getBlockState(new BlockPos(x2, y2, z)).getBlock();

...
public class KawaseBloom {
    ...
    private static void initFramebuffers(float iterations) {
        ...
        while ((float)i <= iterations) {
            Framebuffer currentBuffer = new Framebuffer((int)((double)Client.mc.displayWidth / Math.pow(2.0, i)), (int)((double)Client.mc.displayHeight / Math.pow(2.0, i)), true);
    ...
    public static void renderBlur(int framebufferTexture, int iterations, int offset) {
        int i;
        if (currentIterations != iterations || KawaseBloom.framebuffer.framebufferWidth != Client.mc.displayWidth || KawaseBloom.framebuffer.framebufferHeight != Client.mc.displayHeight) {

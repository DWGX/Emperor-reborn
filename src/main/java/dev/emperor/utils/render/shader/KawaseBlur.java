package dev.emperor.utils.render.shader;

import dev.emperor.Client;
import dev.emperor.utils.render.RenderUtil;
import dev.emperor.utils.render.ShaderUtil;
...
public class KawaseBlur {
    ...
    private static void initFramebuffers(float iterations) {
        ...
        while ((float)i <= iterations) {
            Framebuffer currentBuffer = new Framebuffer((int)((double)Client.mc.displayWidth / Math.pow(2.0, i)), (int)((double)Client.mc.displayHeight / Math.pow(2.0, i)), false);
    ...
    public static void renderBlur(int stencilFrameBufferTexture, int iterations, int offset) {
        int i;
        if (currentIterations != iterations || KawaseBlur.framebuffer.framebufferWidth != Client.mc.displayWidth || KawaseBlur.framebuffer.framebufferHeight != Client.mc.displayHeight) {
            KawaseBlur.initFramebuffers(iterations);
            currentIterations = iterations;
        }
        KawaseBlur.renderFBO(framebufferList.get(1), Client.mc.getFramebuffer().framebufferTexture, kawaseDown, offset);

...
import dev.emperor.module.modules.movement.NoSlow;
    ...
    protected void damageEntity(DamageSource damageSrc, float damageAmount) {
        if (!this.isEntityInvulnerable(damageSrc)) {
            if (!damageSrc.isUnblockable() && this.isBlocking() && damageAmount > 0.0f) {
                ...
                if (((Boolean)NoSlow.blockingDamageAmountDebug.getValue()).booleanValue()) {
                    DebugUtil.log("[Debug]Blocking-damageAmount:" + damageAmount);
                }
            }
